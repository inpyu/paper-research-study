...
파서 품질의 기준선. **손으로 채운다.** 프롬프트나 파서를 고칠 때마다 여기에 대조한다.

## 1. 파서가 뽑은 개념 (자동)
아래는 `parse_notes.py`가 노트에서 뽑은 개념이다. 각 항목 앞을 표시한다.
...
## 2. 빠진 개념 (손으로 추가)
내가 아는데 위 목록에 없는 개념을 여기에 적는다. 이게 파서의 재현율을 재는 기준이다.

- [ ] (예: chunked prefill)
-

## 3. G3 상위 10개 판정
`gaps.py`가 "언급만 되고 정의된 적 없다"고 지목한 것들.
...
> **완료 기준**: 3절에서 `[x]`가 7개 이상이면 M1 통과. 미만이면 파서 규칙을 고친다.
