...
## M2.5 — 레포 위키  ◐ 진행 중
- [x] tree-sitter 전환 — 심볼 939→1837개, 정확한 함수 범위, **호출 간선 7,235개**
- [x] 호출 이름 해석: 같은 파일 → 같은 디렉터리 → 유일 정의 (모호하면 간선을 만들지 않음)
- [x] `trace.py` — **W2 실행 경로** 3종(prefill 본 루프 / 연속 배칭 / 워커 루프)
- [x] W1 파일 · W3 플래그 · W9 연대기 · 학습경로/가설 화면
- [x] G4·G5 문서화 갭
- [x] W4 실험 카드 — run-id 16개(manifest·hosts.tsv·results.tsv·raw 로그·경고)
- [x] W6 수치 대사전 — 수치 1,257개 + G7 출처 없는 측정값 + G8 불일치 후보
- [x] G6 고아 실험 10/16 · G7 577개 · G8 7건
- [x] W5 로그 사전 — 로그 6,171줄 → 템플릿 125종, 코드 출력지점 68종 연결, **노트 설명 3종**
- [x] W7 열린 질문 — 코드 TODO/FIXME 11 · 노트 미결 12 · 실험 2 = 25건
- [x] W8 용어 역인덱스 — 167개 용어(노트·심볼·로그·실험·논문 한 화면)
- [ ] 출처 배지(`노트 인용` / `파싱 사실` / `생성된 서술`)
- [ ] W1 파일 페이지의 서술 생성 (M4.5)
...
## M4.7 — 기초 커리큘럼  ◐ 진행 중
연구 용어 104개는 전부 이 연구 고유의 것이라 **그 아래 단계가 비어 있었다.**
- [x] `curriculum.py` — 딥러닝 기초를 단계별로 설계, 연구 용어로 `leads_to` 연결
- [x] `explain_foundation.py` — 기초 개념 본문 생성(무엇/왜/어떻게/예시/이 연구에서)
- [x] 개념 탭 학습 순서에 기초 단계를 앞에 배치
- [ ] 단계별 진도 체크 · 퀴즈
